...
export default {
  ...
  methods: {
    ...
    changeFacetFilter(e, facetName) {
      this.$emit("filter-change", { name: facetName, value: e.target.value });
    },
    getTerms(facet) {
    ...
    filterChange({ name, value, checked }) {
      // eslint-disable-next-line no-console
      console.log("Filter class", name, value, checked);
      const query = modifyQuery(name, value, this.$route.query, checked);
    ...
    clearFacet(name) {
      this.pushRouter(
        []
          ...
          .reduce(
            (result, val) => modifyQuery(name, val, result, false),
            this.$route.query
          )
      );
    },
